...
export function PdfUpload({ isParsing, progress, pageCount, onFileSelected }: PdfUploadProps) {
  ...
  return (
    <section className="border-b border-slate-200 bg-slate-50 px-3 py-2">
      ...
      >
        <p className="text-xs font-medium text-slate-700">Upload PDF for chat context</p>
        <p className="mt-1 text-[11px] text-slate-500">Drop a file here or choose one from disk.</p>
        <div className="mt-2">
          <button
            type="button"
      ...
      {pageCount ? <p className="mt-2 text-[11px] text-slate-600">Parsed page count: {pageCount}</p> : null}
    </section>
  );
}
